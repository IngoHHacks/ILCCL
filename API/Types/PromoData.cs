...
public class PromoData
{
    ...
    {
        if (UseCharacterNames && SurpriseEntrantsNames.Contains(character.fmq.name))
        {
            return true;
        }
        else if (SurpriseEntrants.Contains(character.fmn))
        {
            return true;
        }
        return false;
    ...
    public Character GetCharacterForCmd(string arg)
    {
        if(UseCharacterNames)
        {
            ...
            {
                return Players.gbe[id].fmq;
            }
            return Players.gbe[0].fmq;
        }
        return UnmappedPromo.gbz[int.Parse(arg)];
    }
