...
internal class SaveRemapper
{
    internal static void PatchCustomContent(ref SaveData saveData)
    {
        ...
        for (int i = 0; i < oldAnims.Count; i++)
        {
            ...
            for (int j = 1; j < saveData.savedChars.Length; j++)
            {
                ...
                foreach (var moveset in saveData.savedChars[j].moveset)
                {
                    for (int k = 0; k < moveset.moveAttack.Length; k++)
                    {
                        if (moveset.moveAttack[k] == i + 1000000)
                        {
                            moveset.moveAttack[k] = index == -1 ? DefaultMove('a', k) : index + 1000000;
                        }
                    }

                    for (int k = 0; k < moveset.moveCrush.Length; k++)
                    {
                        if (moveset.moveCrush[k] == i + 1000000)
                        {
                            moveset.moveCrush[k] = index == -1 ? DefaultMove('c', k) : index + 1000000;
                        }
                    }

                    for (int k = 0; k < moveset.moveFront.Length; k++)
                    {
                        if (moveset.moveFront[k] == i + 1000000)
                        {
                            moveset.moveFront[k] = index == -1 ? DefaultMove('f', k) : index + 1000000;
                        }
                    }

                    for (int k = 0; k < moveset.moveBack.Length; k++)
                    {
                        if (moveset.moveBack[k] == i + 1000000)
                        {
                            moveset.moveBack[k] = index == -1 ? DefaultMove('b', k) : index + 1000000;
                        }
                    }

                    for (int k = 0; k < moveset.moveGround.Length; k++)
                    {
                        if (moveset.moveGround[k] == i + 1000000)
                        {
                            moveset.moveGround[k] = index == -1 ? DefaultMove('g', k) : index + 1000000;
                        }
                    }

                    for (int k = 0; k < moveset.taunt.Length; k++)
                    {
                        if (moveset.taunt[k] == i + 1000000)
                        {
                            moveset.taunt[k] = index == -1 ? DefaultMove('t', k) : index + 1000000;
                        }
                    }
                }
            }
    ...
    public static void FixBrokenSaveData()
    {
        ...
        for (int i = 1; i < saveData.boss.Length; i++)
        {
            int bossIndex = saveData.boss[i];
            if (bossIndex > numChars)
            {
                LogWarning(
                    $"Boss character index {bossIndex} is out of bounds. Resetting to 1.");
                saveData.boss[i] = 1;
            }
            int oldBossIndex = saveData.oldBoss[i];
            if (oldBossIndex > numChars)
            {
                LogWarning(
                    $"Old boss character index {oldBossIndex} is out of bounds. Resetting to 1.");
                saveData.oldBoss[i] = 1;
            }
        }
        ...
        if (saveData.history.Length != numChars)
        {
            LogWarning(
                $"History array length {saveData.history.Length} does not match number of characters {numChars}. Fixing.");
            int oldLength = saveData.history.Length;
            Array.Resize(ref saveData.history, numChars);
            if (oldLength < numChars)
            {
                for (int i = oldLength; i < numChars; i++)
                {
                    saveData.history[i] = 0;
                }
            }
        }
