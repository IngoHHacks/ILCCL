...
public class CharacterUtils
{
    ...
    public static void DeleteCharacter(int id)
    {
        ...
        try
        {
            ...
            for (int i = id; i < Characters.history.Length - 1; i++)
            {
                Characters.history[i] = Characters.history[i + 1];
            }
            
            for (int i = 1; i < World.boss.Length; i++)
            {
                if (World.boss[i] == id)
                {
                    World.boss[i] = 1;
                }
                else if (World.boss[i] > id)
                {
                    World.boss[i]--;
                }
                if (World.oldBoss[i] == id)
                {
                    World.oldBoss[i] = 1;
                }
                else if (World.oldBoss[i] > id)
                {
                    World.oldBoss[i]--;
                }
            }
    ...
    public static int CreateRandomCharacter()
    {
        try
        {
            ...
            Characters.c[Characters.no_chars].teamName = "";
            ...
            LogInfo($"New character created: {Characters.c[Characters.no_chars].name}");
