...
internal class ModTabPatch
{
     ...
     */
    [HarmonyPatch(typeof(bs), nameof(bs.Start))]
    [HarmonyTranspiler]
    public static IEnumerable<CodeInstruction> bs_Start(IEnumerable<CodeInstruction> instructions)
    {
        foreach (CodeInstruction instruction in instructions)
        {
            if (instruction.opcode == OpCodes.Call && (MethodInfo)instruction.operand == AccessTools.Method(typeof(q), nameof(q.ss)))
            {
    ...
    [HarmonyPatch(typeof(bs), nameof(bs.Start))]
    [HarmonyPrefix]
    public static void bs_Start(bs __instance)
    {
     ...
     */
    [HarmonyPatch(typeof(UnmappedMenus), nameof(UnmappedMenus.sv))]
    [HarmonyPostfix]
    public static void Menus_sv()
    {
        mReset = true;
    }
     ...
     */
    [HarmonyPatch(typeof(bs), nameof(bs.Update))]
    [HarmonyPostfix]
    public static void bs_Update(bs __instance)
    {
        if (MappedMenus.tab == 6)
        {
            ...
            if (diff != 0)
            {
                ...
                if (info == null)
                {
                    ...
                    Object.Destroy(info.transform.Find("Highlight").gameObject);
     ...
     */
    [HarmonyPatch(typeof(UnmappedMenus), nameof(UnmappedMenus.si))]
    [HarmonyPrefix]
    public static void Menus_si()
    {
