...
using ILCCL.Content;
using ILCCL.Updates;
using static ILCCL.Utils.NumberFormatUtils;
...
internal class ContentPatch
{
     ...
     */
    [HarmonyPatch(typeof(UnmappedGlobals), nameof(UnmappedGlobals.ob))]
    [HarmonyPrefix]
    public static bool Globals_ob(ref Object __result, string a, string b)
    {
        if (a.StartsWith("Music"))
        {
            if (b.StartsWith("Theme") && int.Parse(b.Substring(5)) > VanillaCounts.Data.MusicCount)
            {
                __result = CustomClips[int.Parse(b.Substring(5)) - VanillaCounts.Data.MusicCount - 1].AudioClip;
                return false;
            }
        }
        else if (a.StartsWith("Costumes"))
        {
            int numberIndex = 0;
            while (numberIndex < b.Length && !char.IsDigit(b[numberIndex]))
            {
                numberIndex++;
            }

            if (numberIndex == 0 || numberIndex == b.Length)
            {
                return true;
            }

            string prefix = b.Substring(0, numberIndex);
            int index = 0;
            if (int.TryParse(b.Substring(numberIndex), out index))
            {
     ...
     */
    [HarmonyPatch(typeof(UnmappedTextures), nameof(UnmappedTextures.bzs))]
    [HarmonyPostfix]
    public static void Textures_bzs()
    {
        VanillaCounts.Data.MaterialCounts = UnmappedTextures.hak.ToList();
        VanillaCounts.Data.FleshCounts = UnmappedTextures.hal.ToList();
        VanillaCounts.Data.ShapeCounts = UnmappedTextures.haj.ToList();
        ...
        VanillaCounts.Data.MaterialCounts[34] = VanillaCounts.Data.ShapeCounts[34];
        VanillaCounts.Data.BodyFemaleCount = UnmappedTextures.han;
        VanillaCounts.Data.FaceFemaleCount = UnmappedTextures.ham;
        VanillaCounts.Data.SpecialFootwearCount = UnmappedTextures.har;
        VanillaCounts.Data.TransparentHairMaterialCount = UnmappedTextures.hao;
        VanillaCounts.Data.TransparentHairHairstyleCount = UnmappedTextures.hap;
        VanillaCounts.Data.KneepadCount = UnmappedTextures.haq;
    ...
    */
     ...
     */
    [HarmonyPatch(typeof(UnmappedPlayer), nameof(UnmappedPlayer.zj))]
    [HarmonyPrefix]
    public static void Player_zj_Pre(ref UnmappedPlayer __instance, ref int a)
    {
        if (a >= 4 && a <= 7) {
            return;
        }
        if (__instance.fth.shape[a] > VanillaCounts.Data.ShapeCounts[a] ||
           (a == 17 && -__instance.fth.shape[a] > VanillaCounts.Data.TransparentHairHairstyleCount))
        {
            temp = __instance.fth.shape[a];
            __instance.fth.shape[a] = 1;
        }
    }
    ...
    [HarmonyPatch(typeof(UnmappedPlayer), nameof(UnmappedPlayer.zj))]
    [HarmonyPostfix]
    public static void Player_zj_Post(ref UnmappedPlayer __instance, int a)
    {
        if (temp != -1)
        {
            __instance.fth.shape[a] = temp;
            var limb = a;
            try
            {
                if (__instance.fth.shape[limb] > VanillaCounts.Data.ShapeCounts[limb] ||
                    (limb == 17 &&
                    -__instance.fth.shape[limb] > VanillaCounts.Data.TransparentHairHairstyleCount))
                {
                    int shape = __instance.fth.shape[limb] > 0
                        ? __instance.fth.shape[limb] - VanillaCounts.Data.ShapeCounts[limb] - 1
                        : -__instance.fth.shape[limb] - VanillaCounts.Data.TransparentHairHairstyleCount - 1;
                    if (CustomCostumes.Values.Any(x => x.InternalPrefix.Contains("shape" + limb)))
                    {
                        ...
                        if (mesh != null)
                        {
                            ...
                            //{
                                __instance.fti[limb].GetComponent<MeshFilter>().mesh = mesh;
                                __instance.fti[limb].GetComponent<MeshRenderer>().material = new Material(Shader.Find("Custom/My Solid"));
                            //}
                            //else
                            //{
                                //Feel it needs something like below but can't get it working
                            //    __instance.fti[limb].GetComponent<SkinnedMeshRenderer>().sharedMesh = mesh;
                            //}
                            if (meta.ContainsKey("scale"))
                            {
                                if (meta["scale"].Contains(","))
                                {
                                    ...
                                    __instance.fti[limb].transform.localScale = new Vector3(
                            ...
                            if (meta.ContainsKey("position"))
                            {
                                ...
                                __instance.fti[limb].transform.localPosition = new Vector3(
                            ...
                            if (meta.ContainsKey("rotation"))
                            {
                                ...
                                __instance.fti[limb].transform.localRotation = Quaternion.Euler(
            ...
            {
                LogError(e);
                LogError("Limb: " + limb + " (" + __instance.fth.shape[limb] + ")");
            }
            temp = -1;
        }
    }
     ...
     */
    [HarmonyPatch(typeof(UnmappedTextures), nameof(UnmappedTextures.cah))]
    [HarmonyPostfix]
    public static void Textures_cah(UnmappedPlayer a)
    {
        FixMeshes(a);
    }
     ...
     */
    [HarmonyPatch(typeof(UnmappedPlayer), nameof(UnmappedPlayer.zm))]
    [HarmonyPostfix]
