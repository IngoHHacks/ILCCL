...
internal class MusicSelectMenuPatch
{
     ...
     */
    [HarmonyPatch(typeof(UnmappedMenus), nameof(UnmappedMenus.sd))]
    [HarmonyPostfix]
    public static void Menus_sd()
    {
        ...
        for (MappedMenus.cyc = 1; MappedMenus.cyc <= MappedMenus.no_menus; MappedMenus.cyc++)
        {
            ...
            else if (UnmappedMenus.Control() > 0 && controller.type > 1 && MappedMenus.cyc == MappedMenus.foc)
            {
        ...
        foreach (CodeInstruction instruction in instructions)
        {
            ...
            if (prev2 != null) {
                if (prev2.opcode == OpCodes.Ldsfld && (FieldInfo)prev2.operand ==
                    AccessTools.Field(typeof(UnmappedMenus), nameof(UnmappedMenus.fkn)))
                {
                    if (prev.opcode == OpCodes.Ldc_I4_1)
                    {
                        yield return new CodeInstruction(OpCodes.Ldsfld,
                            AccessTools.Field(typeof(UnmappedMenus), nameof(UnmappedMenus.fid)));
                        yield return new CodeInstruction(OpCodes.Ldc_I4_0);
                        yield return new CodeInstruction(instruction);
                    }
                } else if (prev2.opcode == OpCodes.Ldsfld && (FieldInfo)prev2.operand ==
                    AccessTools.Field(typeof(Sound), nameof(Sound.grg)))
                {
                    if (prev.opcode == OpCodes.Ldsfld && (FieldInfo)prev.operand ==
                        AccessTools.Field(typeof(Sound), nameof(Sound.gre)))
                    {
                        yield return new CodeInstruction(OpCodes.Ldsfld,
                            AccessTools.Field(typeof(UnmappedMenus), nameof(UnmappedMenus.fid)));
    ...
    [HarmonyTranspiler]
    public static IEnumerable<CodeInstruction> Menus_cds(IEnumerable<CodeInstruction> instructions)
    {
        CodeInstruction prev = null;
        CodeInstruction prev2 = null;
        int screen = 0;
        foreach (CodeInstruction instruction in instructions)
        {
            ...
            if (prev != null && prev.opcode == OpCodes.Ldsfld && (FieldInfo)prev.operand ==
                AccessTools.Field(typeof(UnmappedMenus), nameof(UnmappedMenus.fhy)))
            {
