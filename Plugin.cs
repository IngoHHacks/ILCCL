...
public class Plugin : BaseUnityPlugin
{
    ...
    public static string[] PreReleaseReasons = { "Beta" };
    ...
    private void Awake()
    {
        try
        {
            ...
            PersistentDataOverride = this.Config.Bind("General", "PersistentDataOverride", "",
                "Override the persistent data path, which is used for storing the cache and mappings. This is useful if you want to use a different drive for the cache and mappings, since they can get quite large. Note that this will not move the existing cache and mappings, so you will have to move them manually. Leave this empty to use the default persistent data path: " + Path.Combine(Application.persistentDataPath, "ILCCL").Replace('/', '\\'));
            
            PersistentDataPath = string.IsNullOrEmpty(PersistentDataOverride.Value) ? Path.Combine(Application.persistentDataPath, "ILCCL") : PersistentDataOverride.Value;
            ...
            UseFullQualityTextures = this.Config.Bind("General", "UseFullQualityTextures", false,
                "(EXPERIMENTAL) Allow ILCCL to use the full resolution textures without scaling them down (the game will still change the aspect ratio to fit the texture).");
            ...
            EnableCharacterSearchScreen = this.Config.Bind("General", "EnableCharacterSearchScreen", true,
                "Enable the character search screen in the roster menu.");
            ...
            {
                throw new DirectoryNotFoundException("Data directory not found. Please make sure you copied the Data folder to the same directory as the ILCCL DLL.");
            }
            Locations.LoadData();
                ...
                else
                {
                    foreach (string reason in PreReleaseReasons)
                    {
                        switch (reason)
                        {
                            ...
                            case "Beta":
                                LogWarning(
                                    "This is a beta version that is still being tested and may contain bugs.");
                                break;
                            default:
...
#pragma warning restore CS0162 // Unreachable code detected
            ...
            if (GameVersion != PluginVersion)
            {
                if (GameVersion > PluginVersion)
                {
                    LogWarning($"Your game version ({GameVersion}) is newer than ILCCL's target version ({PluginVersion}). Keep in mind that it may not work as intended on this version. Check the Steam Workshop page for updates.");
                }
                else
                {
                    LogWarning($"Your game version ({GameVersion}) is older than ILCCL's target version ({PluginVersion}). Keep in mind that it may not work as intended on this version. Check for updates in the game's properties on Steam.");
                }
            }
            ...
            {
                Application.OpenURL("https://ingoh.net/support-ILCCL");
